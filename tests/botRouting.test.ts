...
describe('handleMessageCreate', () => {
  ...
  it('routes owner mixed Korean and Arabic tarot selections before watched-channel TTS', async () => {
    const commands = createPrefixCommands();
    const tarotSessions = new TarotSessionStore(10_000);
    tarotSessions.start({ guildId: 'guild-1', channelId: 'channel-1', userId: 'user-1' }, {
      topic: '컨디션 변화',
      spreadCount: 5,
      requesterDisplayName: '테스터',
      seed: 'seed-owner-mixed-korean-arabic'
    }, 1_000);
    const context = makeContext({
      tarotSessions,
      agentRuntime: { run: vi.fn(async () => ({ kind: 'clarify', message: '같은 번호가 있어요. 서로 다른 번호 5개를 다시 골라주세요.' })) }
    });
    context.voiceSettings.setAiChannelId('guild-1', 'channel-1');
    const message = makeMessage('일 이 삼 3 6', { createdTimestamp: 1_500 });
    ...
    expect(context.agentRuntime.run).toHaveBeenCalledWith(
      message,
      '일 이 삼 3 6',
      expect.objectContaining({ tarotPending: expect.objectContaining({ topic: '컨디션 변화', spreadCount: 5 }) })
    );
    expect(context.aiChat.handlePrompt).not.toHaveBeenCalled();
    expect(context.voice.enqueueMessage).not.toHaveBeenCalled();
    expect(message.reply).toHaveBeenCalledWith(expect.objectContaining({ content: '같은 번호가 있어요. 서로 다른 번호 5개를 다시 골라주세요.' }));
  });
  ...
  it('lets logging guild admins make the bot leave the source guild from its log channel topic id', async () => {
    ...
    const loggingGuild = { id: 'log-guild', name: '로그서버', leave: vi.fn(async () => undefined) };
    const context = makeContext({
      settings: {
        ttsReadBotMessages: false,
        ttsMaxChars: 500,
    ...
    const message = makeMessage('!서버탈퇴', {
      guildId: 'log-guild',
      channel: {
        id: 'log-channel',
        type: ChannelType.GuildText,
        topic: 'Source guild: 떠날서버 (123456789012345678)',
      ...
      client: {
        ...
        guilds: {
          cache: new Map([
            ['log-guild', loggingGuild],
            ['123456789012345678', targetGuild]
          ]),
          fetch: vi.fn(async (id: string) => id === '123456789012345678' ? targetGuild : null)
        }
      },
      member: {
        displayName: '관리자',
    ...
    expect(loggingGuild.leave).not.toHaveBeenCalled();
    expect(message.reply).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('rejects server leave outside the logging guild', async () => {
    ...
    const message = makeMessage('!서버탈퇴 123456789012345678', {
      guildId: 'guild-1',
      client: {
        user: { id: 'bot-1', username: 'ChococoBot' },
        guilds: {
    ...
    expect(targetLeave).not.toHaveBeenCalled();
    expect(message.reply).toHaveBeenCalledWith(expect.objectContaining({ content: '로그 서버에서만 사용할 수 있어요...' }));
  });

  it('does not let AI planner execute the logging-guild server leave command', async () => {
    const commands = createPrefixCommands();
    const targetLeave = vi.fn(async () => undefined);
    const context = makeContext({
      settings: {
      ...
      aiCommandPlanner: {
        plan: vi.fn(async () => ({ kind: 'command', query: '서버탈퇴 123456789012345678' }))
      }
    });
    const message = makeMessage('!? 123456789012345678 서버에서 나가', {
      guildId: 'log-guild',
      client: {
        user: { id: 'bot-1', username: 'ChococoBot' },
        guilds: {
    ...
    expect(targetLeave).not.toHaveBeenCalled();
    expect(message.reply).toHaveBeenCalledWith(expect.objectContaining({
      content: '어떤 명령을 실행해야 할지 확실하지 않아요... 조금 더 구체적으로 말해 주세요...'
    }));
  });
